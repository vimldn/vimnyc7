...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // If full-text is empty, fall back to a prefix match (faster than contains).
    // Heuristic: if the user starts with a building number, prefix matching is usually what they expect.
    if (!Array.isArray(data) || data.length === 0) {
      const prefixLike = qUpper.match(/^\d/) ? `${qUpper}%25` : `%25${qUpper}%25`
      const fallbackQuery = `https://data.cityofnewyork.us/resource/64uk-42ks.json?$where=address LIKE '${prefixLike}'&$limit=12&$select=bbl,address,borough,zipcode,unitsres&$order=unitsres DESC`
